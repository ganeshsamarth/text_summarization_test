...
#####################define data sources#################
#########################################################
CNN_data="/home/pbu/Downloads/cnn"
...
def parsetext(dire,category,filename):
    with open(dire+'/'+category+'/'+filename,'r',encoding="Latin-1") as readin:
...
def load_data(dire,category):
    ...
    return os.listdir(dire+'/'+category)
...
for k in range(len(filenames[:1000])):
        if k%2==0:
            try:
